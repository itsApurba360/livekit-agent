...
uv sync
```
*(Note: If you run into `onnxruntime` resolution warnings on macOS Intel during local sync, you can alternatively initialize with `uv pip install livekit livekit-agents requests pytest python-dotenv` to execute code and test logic without media plugins).*
...
4. **Port Allocation**: Since LiveKit agent workers run as background workers connecting *outbound* to the LiveKit Server via WebSockets, **you do not need to expose any ports** or configure inbound routing rules on Docploy.
